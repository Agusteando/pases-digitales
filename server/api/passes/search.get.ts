...
export default defineEventHandler(async (event) => {
  ...
  let sql = `SELECT id, employee_name, date, fecha_fin, time, comentarios, category_id, status, user, plantel, regreso, hora_regreso, IMSS, tipo_incapacidad, tipo_permiso, permanent_weekdays, processed, horario_entrada, horario_salida FROM hr_entries WHERE 1=1`
  const params: any[] = []
