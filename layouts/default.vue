<template>
  <div class="min-h-[100dvh] flex flex-col md:flex-row">
    ...
    <nav class="md:hidden fixed bottom-0 w-full bg-white/90 backdrop-blur-md border-t border-slate-200 pb-safe pt-2 px-4 flex justify-around items-center z-50 shadow-lg">
      <NuxtLink to="/" class="px-4 py-2 flex flex-col items-center gap-1 transition-colors" active-class="text-brand-600" :class="$route.path === '/' ? '' : 'text-slate-500'">
        <Plus class="w-6 h-6" />
      ...
      <button @click="handleLogout" class="px-4 py-2 flex flex-col items-center gap-1 text-slate-500 transition-colors hover:text-red-600">
        <LogOut class="w-6 h-6" />
