<template>
  <div class="p-6 md:p-10 max-w-7xl mx-auto h-full overflow-y-auto custom-scrollbar relative z-10 flex flex-col">
    ...
    <header class="mb-6 shrink-0">
      ...
      <p class="text-slate-500 mt-2 text-sm font-medium">Administración del directorio de planteles y enrutamiento de notificaciones.</p>
    </header>
      ...
      <div class="glass-card p-5 rounded-2xl mb-6 bg-white border border-slate-200 shadow-sm shrink-0">
        <div class="flex items-center justify-between mb-4">
          <div>
            ...
            <p class="text-xs text-slate-500 font-medium mt-0.5">Define condiciones específicas para disparar notificaciones estructuradas.</p>
          </div>
        </div>
        ...
        <form @submit.prevent="saveRule" class="grid grid-cols-1 lg:grid-cols-12 gap-4 items-end">
          ...
          <div class="space-y-1.5 lg:col-span-3">
            <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider">Y el Puesto es</label>
            <select v-model="ruleForm.condition_puesto" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 focus:border-brand-500 outline-none text-sm font-medium shadow-sm transition-colors bg-white">
    ...
    <div v-if="activeTab === 'directory'" class="flex-1 flex flex-col min-h-0 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div class="flex items-center justify-between mb-4 shrink-0">
        <div>
          ...
          <p class="text-xs text-slate-500">Contactos operativos provenientes de Google Workspace.</p>
        </div>
      ...
      <div v-else-if="!contacts || contacts.length === 0" class="flex-1 flex flex-col items-center justify-center text-slate-400 py-12 glass-card rounded-2xl border border-slate-200 border-dashed">
        ...
        <p class="text-sm font-medium text-slate-500 mt-1 text-center max-w-md">Vincula cuentas institucionales para dotar de responsables a cada plantel.</p>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form @submit.prevent="saveContact" id="addContactForm" class="p-6 space-y-5">
          ...
          <div class="space-y-1.5 relative">
            <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider">Buscar Contacto (Workspace)</label>
            <input v-model="gwSearchQuery" @input="searchGw" placeholder="Nombre o correo..." class="w-full px-3 py-2.5 rounded-xl border border-slate-300 focus:border-brand-500 outline-none text-sm font-medium shadow-sm transition-colors" autocomplete="off" />
            
            <div v-if="gwResults.length" class="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 shadow-xl rounded-xl z-50 max-h-48 overflow-y-auto custom-scrollbar py-1">
              <button type="button" v-for="res in gwResults" :key="res.email" @click="selectGw(res)" class="w-full text-left px-4 py-2 hover:bg-slate-50 border-b border-slate-100/50 flex items-center gap-3 last:border-0 transition-colors">
                 <img v-if="res.photoUrl" :src="res.photoUrl" class="w-8 h-8 rounded-full border border-slate-200" />
                 <div v-else class="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-slate-400 text-xs">{{ res.name.slice(0,2).toUpperCase() }}</div>
                 <div class="flex-1 min-w-0">
                   <p class="text-sm font-bold text-slate-800 truncate">{{ res.name }}</p>
                   <p class="text-[10px] text-slate-500 truncate">{{ res.email }}</p>
                 </div>
              </button>
            </div>
            
            <div v-if="form.email" class="mt-3 p-3 bg-brand-50 border border-brand-200 rounded-xl flex items-center gap-3 shadow-sm">
              <div class="w-10 h-10 rounded-full bg-white border border-brand-200 overflow-hidden shrink-0">
                <img v-if="selectedGwPhoto" :src="selectedGwPhoto" class="w-full h-full object-cover" />
                <div v-else class="w-full h-full flex items-center justify-center font-bold text-brand-400 text-xs">{{ selectedGwName.slice(0,2).toUpperCase() }}</div>
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-sm font-bold text-brand-900 truncate">{{ selectedGwName }}</p>
                <p class="text-xs text-brand-700 truncate">{{ form.email }}</p>
              </div>
              <button type="button" @click="clearGw" class="text-brand-400 hover:text-red-500 hover:bg-red-50 p-1.5 rounded-lg transition-colors focus:outline-none">
                <X class="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        <footer class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          <div class="flex items-center gap-2 text-slate-400">
            ...
            <span class="text-[10px] font-bold">Datos estructurados.</span>
          </div>
          <div class="flex gap-2">
            ...
            <button type="submit" form="addContactForm" :disabled="isSaving || !form.email" class="px-5 py-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-bold rounded-lg transition-all shadow-sm flex items-center gap-2 outline-none disabled:opacity-70">
...
const isAdmin = computed(() => user.value?.email?.includes('admon') || user.value?.email?.includes('tecno') || user.value?.is_admin)
...
const deleteContact = async (id) => {
  if (!confirm('¿Desvincular permanentemente este contacto del directorio?')) return
  try {
...
const saveRule = async () => {
  ...
  try {
    ...
    ruleForm.value = { condition_plantel: 'ALL', condition_puesto: 'ALL', target_type: 'CONTACT', target_val: '' }
    refreshRules()
  } catch (error) {
