        ...
        <div class="flex-1 xl:overflow-y-auto overflow-visible px-6 py-6 md:px-8 custom-scrollbar relative flex flex-col gap-10 min-h-0 xl:shadow-[inset_0_10px_20px_rgba(0,0,0,0.01)]">
          ...
          <div class="relative">
            ...
            <div v-if="selectedEmployees.length > 0" class="flex flex-col gap-3 mt-5">
              <div v-for="emp in selectedEmployees" :key="emp.id" class="flex flex-col w-full bg-white/70 backdrop-blur-md p-4 rounded-[1.25rem] border border-white shadow-sm group transition-all relative overflow-hidden hover:border-white/80 hover:shadow-md">
                <div class="flex items-start justify-between mb-2 relative z-10">
                  ...
                  <button type="button" @click="removeEmployee(emp.id)" class="text-slate-400 hover:text-red-500 bg-white shadow-sm w-7 h-7 flex items-center justify-center rounded-full transition-colors focus:outline-none shrink-0 outline-none border border-white/60">
                ...
                <div v-else class="flex flex-col gap-2 w-full mt-1 relative z-10 pt-3 border-t border-white/60">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button v-else-if="!emp._editingActual" type="button" @click.stop="emp._editingActual = true" class="text-[10px] font-black text-slate-400 hover:text-[#007F92] ml-2 transition-colors flex items-center gap-1 outline-none uppercase tracking-widest bg-white/50 px-2 py-0.5 rounded-md border border-white">
                  ...
                  <div v-if="emp._editingActual" class="p-2 bg-white/60 backdrop-blur-md rounded-xl flex items-center gap-2 animate-in fade-in duration-200 mt-2 border border-white shadow-sm">
                     <div class="flex-1">
                        <select v-model="emp.plantelActual" @change="onPlantelActualSelected(emp)" class="w-full px-2 py-2 rounded-lg border border-white/80 text-xs font-bold text-slate-700 bg-white/80 focus:ring-2 focus:ring-[#007F92]/20 focus:border-[#007F92] outline-none cursor-pointer transition-all shadow-sm">
                           <option v-for="p in plantelesList" :key="p" :value="p">{{ p }}</option>
                        </select>
                     </div>
                     <button type="button" @click="emp._editingActual = false" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-slate-600 bg-white rounded-lg shadow-sm transition-colors border border-white outline-none shrink-0">
                        <XIcon class="w-4 h-4 shrink-0" />
                     </button>
                  </div>
                </div>
          ...
          <div v-if="checkingCoverage" class="py-10 flex flex-col items-center justify-center bg-white/60 backdrop-blur-md rounded-[1.5rem] border border-white shadow-sm">
          ...
          <div v-else-if="!currentCoverageTask && selectedEmployees.length > 0" class="relative animate-in slide-in-from-bottom-4 fade-in duration-500 pb-8">
            ...
            <div class="bg-white/40 backdrop-blur-md p-3 rounded-[2rem] shadow-sm border border-white/60">
          ...
          <div class="flex-1 xl:overflow-y-auto overflow-visible px-5 py-6 md:px-8 custom-scrollbar relative flex flex-col min-h-0 bg-transparent xl:shadow-[inset_0_10px_20px_rgba(0,0,0,0.01)] pb-40 xl:pb-6">
            ...
            <form @submit.prevent class="relative flex flex-col gap-8 py-2">
              ...
              <div v-if="activeScenario.categoryId === 2" class="flex flex-wrap items-center gap-2.5 pb-6 border-b border-white/60">
                <button type="button" @click="applyPreset('now')" class="px-3.5 py-2 bg-white/70 backdrop-blur-md hover:bg-white text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl border border-white transition-colors flex items-center gap-1.5 outline-none shadow-sm">
                  <Clock class="w-3.5 h-3.5" /> Hora actual
                </button>
                <button type="button" @click="applyPreset('transfer')" class="px-3.5 py-2 bg-white/70 backdrop-blur-md hover:bg-white text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl border border-white transition-colors flex items-center gap-1.5 outline-none shadow-sm">
                  <MapPin class="w-3.5 h-3.5" /> Traslado plantel
                </button>
                <button v-if="hasBirthday()" type="button" @click="applyPreset('birthday')" class="px-3.5 py-2 bg-[#F49A6D]/15 backdrop-blur-md hover:bg-[#F49A6D]/25 text-[#D97746] text-[10px] font-black uppercase tracking-widest rounded-xl border border-[#F49A6D]/30 transition-colors flex items-center gap-1.5 outline-none shadow-sm">
              ...
              <div class="relative group">
                ...
                <div v-if="activeScenario.categoryId === 4" class="space-y-4 mb-5 pb-5 border-b border-white/60">
                  <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="space-y-1.5">
                      ...
                      <input type="date" v-model="form.shiftDate" :min="todayDate" class="w-full px-3.5 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white/70 backdrop-blur-sm shadow-sm" />
                    </div>
                    <div class="space-y-1.5">
                      <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Inicio</label>
                      <input type="time" v-model="form.shiftStart" class="w-full px-3.5 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white/70 backdrop-blur-sm shadow-sm" />
                    </div>
                    <div class="space-y-1.5">
                      <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Fin</label>
                      <input type="time" v-model="form.shiftEnd" class="w-full px-3.5 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white/70 backdrop-blur-sm shadow-sm" />
                    </div>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div class="space-y-1.5 col-span-2 sm:col-span-1">
                    ...
                    <input type="date" v-model="form.date" :min="todayDate" required class="w-full px-3.5 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white/70 backdrop-blur-sm shadow-sm" />
                  </div>
                  ...
                  <div v-if="activeScenario.needsEndDate" class="space-y-1.5 col-span-2 sm:col-span-1">
                    ...
                    <input type="date" v-model="form.endDate" :min="todayDate" required class="w-full px-3.5 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white/70 backdrop-blur-sm shadow-sm" />
                  </div>
                  ...
                  <div v-if="activeScenario.needsTime" class="space-y-1.5 col-span-2 sm:col-span-1">
                    ...
                    <input type="time" v-model="form.time" required class="w-full px-3.5 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white/70 backdrop-blur-sm shadow-sm" />
                  </div>
                </div>
                ...
                <div v-if="activeScenario.canReturn" class="mt-5 pt-5 border-t border-white/60 space-y-4">
                  ...
                  <div v-if="form.regreso" class="space-y-2 animate-in fade-in bg-white/60 backdrop-blur-md p-4 rounded-2xl border border-white shadow-sm">
                    ...
                    <input type="time" v-model="form.horaRegreso" required class="w-full px-3.5 py-2.5 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white shadow-inner" />
                  </div>
                </div>
              </div>
              ...
              <div v-if="activeScenario.categoryId === 3 || activeScenario.categoryId === 4 || activeScenario.isMedical || activeScenario.categoryId === 2" class="relative group pt-6 border-t border-white/60">
                ...
                <div v-if="activeScenario.categoryId === 3" class="space-y-3">
                  ...
                  <div class="flex flex-wrap gap-2.5">
                    <button 
                      ...
                      class="px-4 py-2.5 text-[11px] font-black uppercase tracking-widest rounded-xl transition-all border outline-none text-center shadow-sm backdrop-blur-md"
                ...
                <div v-if="activeScenario.categoryId === 4" class="space-y-3">
                  ...
                  <div class="flex flex-col sm:flex-row gap-3">
                    <button 
                      ...
                      class="px-4 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all border outline-none flex-1 text-center shadow-sm backdrop-blur-md"
                ...
                <div v-if="activeScenario.isMedical" class="space-y-5">
                  <div class="space-y-1.5">
                    ...
                    <input type="text" v-model="form.imss" placeholder="Ej. 12345678" class="w-full px-4 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 transition-all bg-white/70 backdrop-blur-sm shadow-sm" />
                  </div>
                  <div class="space-y-3">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                      <button 
                        ...
                        class="px-4 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all border outline-none flex-1 text-center shadow-sm backdrop-blur-md"
                ...
                <div v-if="activeScenario.categoryId === 2" class="mt-5">
                  <div v-if="!showDestino" class="flex">
                    <button type="button" @click="showDestino = true" class="text-[10px] font-black text-slate-500 hover:text-[#007F92] flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-white/70 backdrop-blur-md border border-white hover:bg-white transition-colors shadow-sm outline-none uppercase tracking-widest">
                  ...
                  <div v-else class="p-4 bg-white/60 backdrop-blur-md rounded-2xl border border-white shadow-sm relative animate-in fade-in duration-200">
                    <button type="button" @click="showDestino = false" class="absolute top-3 right-3 text-slate-400 hover:text-slate-600 p-1.5 bg-white rounded-lg hover:bg-slate-50 border border-white/60 transition-colors outline-none shadow-sm">
                       <XIcon class="w-4 h-4" />
                    </button>
                    <label class="block text-[10px] font-black text-[#007F92] uppercase tracking-widest mb-2 px-1">Plantel de destino</label>
                    <select v-model="form.destino" class="w-full px-3.5 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-bold text-slate-800 bg-white/80 cursor-pointer transition-all pr-8 shadow-sm">
              ...
              <div class="relative group pt-6 border-t border-white/60">
                ...
                <div class="space-y-1.5 mb-5">
                  ...
                  <textarea v-model="form.comentarios" rows="3" placeholder="Describe brevemente la razón..." required class="w-full px-4 py-3 rounded-xl border border-white/80 focus:border-[#007F92] focus:ring-2 focus:ring-[#007F92]/20 outline-none text-sm font-medium text-slate-900 resize-none transition-all bg-white/70 backdrop-blur-sm shadow-sm"></textarea>
                </div>
                ...
                <div v-if="activeScenario.categoryId === 3" class="space-y-2 pt-2">
                  ...
                  <div class="relative border border-dashed border-slate-300/80 hover:border-[#007F92]/50 bg-white/50 backdrop-blur-md rounded-2xl p-6 transition-all text-center group cursor-pointer" :class="{'border-[#007F92] border-solid bg-[#007F92]/5': evidenceFile}">
                    ...
                    <div v-else class="flex items-center justify-between z-20 relative px-1">
                      ...
                      <button type="button" @click.stop.prevent="clearEvidence" class="p-2 text-slate-400 hover:text-red-500 bg-white hover:bg-red-50 rounded-xl transition-colors outline-none shrink-0 shadow-sm border border-white/60">
                        <XIcon class="w-5 h-5" />
                      </button>
                    </div>
                  </div>
              ...
              <!-- Opt-In Authorizer -->
              <div v-if="!isAuthorizerForCurrent && selectedEmployees.length > 0" class="p-5 bg-white/70 backdrop-blur-md rounded-[1.5rem] border border-white shadow-sm transition-all animate-in fade-in">
                ...
                <div v-if="form.optInAuthorizer && !myProfile?.phone" class="mt-4 pt-4 border-t border-white/60 animate-in fade-in">
                  ...
                  <div class="flex items-center rounded-xl border border-white/80 overflow-hidden bg-white/80 focus-within:border-[#007F92] focus-within:ring-2 focus-within:ring-[#007F92]/20 shadow-sm">
                     <div class="bg-white/90 border-r border-white px-4 py-3 text-slate-500 font-black text-sm">+52 1</div>
          ...
          <div class="fixed bottom-14 md:bottom-0 left-0 right-0 z-50 p-4 bg-white/95 backdrop-blur-xl border-t border-slate-200/80 shadow-[0_-10px_30px_rgba(0,0,0,0.08)] xl:static xl:bg-white/80 xl:border-0 xl:border-t xl:border-white xl:shadow-[0_-10px_30px_rgba(0,0,0,0.03)] xl:p-6 xl:rounded-none xl:mt-auto shrink-0 flex flex-col gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-stretch">
              <button 
                ...
                class="flex-1 relative group overflow-hidden bg-white/90 backdrop-blur-md hover:bg-white text-slate-700 font-black text-sm rounded-[1.25rem] transition-all border-2 border-white hover:border-white shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed h-14 outline-none"
              >
