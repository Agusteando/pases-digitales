<template>
  <div class="h-full flex flex-col relative z-10">
    <div class="mb-4 shrink-0 flex items-center justify-end gap-3">
      <button @click="openPlantelDefault" :disabled="selectedPlantel === 'ALL'" class="px-4 py-2.5 bg-white border border-slate-200 text-slate-700 text-xs font-black rounded-xl shadow-sm hover:border-brand-300 hover:text-brand-700 transition-all flex items-center gap-2 outline-none disabled:opacity-40 disabled:cursor-not-allowed">
        <Building2 class="w-4 h-4" /> Default del plantel
      </button>
      <button @click="openBulkAssignment" class="px-4 py-2.5 bg-gradient-to-r from-iedis-teal to-iedis-teal-dark text-white text-xs font-black rounded-xl shadow-md hover:shadow-lg transition-all flex items-center gap-2 outline-none">
          ...
          <div class="p-6 border-b border-white/70 bg-white/50 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
              <div class="relative min-w-[260px]">
                ...
                <input v-model="searchQuery" type="search" placeholder="Buscar puesto o autorizador" class="w-full pl-11 pr-4 py-3 rounded-2xl bg-white/80 border border-white focus:border-iedis-teal focus:ring-2 focus:ring-iedis-teal/20 outline-none text-sm font-bold text-slate-800 shadow-sm" />
              </div>
        ...
        <div class="p-8 flex-1 overflow-y-auto custom-scrollbar space-y-8">
          ...
          <section class="space-y-3">
            ...
            <div v-if="!form.email" class="relative">
              <Search class="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
              <input v-model="gwSearchQuery" @input="searchGw" placeholder="Buscar por nombre" class="w-full pl-11 pr-4 py-4 rounded-2xl bg-white border border-slate-200 focus:border-iedis-teal focus:ring-2 focus:ring-iedis-teal/20 outline-none text-sm font-bold text-slate-800" />
