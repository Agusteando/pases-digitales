    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-white/20">
      ...
      <div class="p-6 md:p-8 bg-white min-h-[200px] flex flex-col justify-center">
        <div v-if="setupForm.saving" class="py-6 flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-300">
          ...
          <p class="text-sm font-bold text-slate-500">Completando paso...</p>
        </div>

        <div v-else class="animate-in fade-in duration-300">
          <p class="text-sm font-medium text-slate-600 mb-6 leading-relaxed">
            Para continuar con el pase de <strong class="text-slate-900 font-black">{{ employeeFirstName }}</strong>, necesitamos confirmar un dato de <strong class="text-slate-900 font-black">{{ plantel }}</strong>.
          </p>

          <!-- State 1: Select Person -->
          <div v-if="setupForm.internalStep === 'SELECT_PERSON'" class="space-y-4">
            <label class="block text-sm font-bold text-slate-700">Selecciona a la persona responsable para continuar.</label>
            <div class="relative">
